            ...
            using (NpgsqlCommand cmd = new NpgsqlCommand())
            {
                ...
                using (NpgsqlDataReader reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        l_LigneCommande = new cls_LigneCommande(l_IDLigneCommande, l_NumeroLigne, l_Quantite, Program.Modele.ListeCommandes[l_IDCommande],
                            l_IDEtat, l_IDProduit, l_IDTva);
            ...
            using (NpgsqlCommand cmd = new NpgsqlCommand())
            {
                ...
                using (NpgsqlDataReader reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        l_LigneCommande = new cls_LigneCommande(l_IDLigneCommande, l_NumeroLigne, l_Quantite,
                            Program.Modele.ListeCommandes[l_IDCommande], l_IDEtat, l_IDProduit, l_IDTva);
                        l_ListeLignesCommande.Add(l_IDLigneCommande, l_LigneCommande);
                    }
                }
            }
